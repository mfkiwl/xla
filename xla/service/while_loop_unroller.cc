...
static StatusOr<std::unique_ptr<HloComputation>> LoopUnrollHelper(
  ...
  auto induction_var_hlo = while_op->operand(0)->operand(indvar_idx);
      ...
      for (HloInstruction* indvar_use : body_inst->users()) {
        for (int64_t i = 0; i < indvar_use->operand_count(); ++i) {
          ...
          if (indvar_use_operand == body_inst) {
            HloInstruction* constant_indvar;
            switch (induction_var_hlo->shape().element_type()) {
              case S8:
                constant_indvar = while_body_clone->AddInstruction(
                    HloInstruction::CreateConstant(LiteralUtil::CreateR0(
                        static_cast<int8_t>(induction_value))));
                break;
              case S16:
                constant_indvar = while_body_clone->AddInstruction(
                    HloInstruction::CreateConstant(LiteralUtil::CreateR0(
                        static_cast<int16_t>(induction_value))));
                break;
              case S32:
                constant_indvar = while_body_clone->AddInstruction(
                    HloInstruction::CreateConstant(LiteralUtil::CreateR0(
                        static_cast<int32_t>(induction_value))));

                break;
              case S64:
                constant_indvar = while_body_clone->AddInstruction(
                    HloInstruction::CreateConstant(LiteralUtil::CreateR0(
                        static_cast<int64_t>(induction_value))));
                break;
              case U8:
                constant_indvar = while_body_clone->AddInstruction(
                    HloInstruction::CreateConstant(LiteralUtil::CreateR0(
                        static_cast<uint8_t>(induction_value))));
                break;
              case U16:
                constant_indvar = while_body_clone->AddInstruction(
                    HloInstruction::CreateConstant(LiteralUtil::CreateR0(
                        static_cast<uint16_t>(induction_value))));
                break;
              case U32:
                constant_indvar = while_body_clone->AddInstruction(
                    HloInstruction::CreateConstant(LiteralUtil::CreateR0(
                        static_cast<uint32_t>(induction_value))));
                break;
              case U64:
                CHECK(false);
                break;
              default:
                return InternalError("literal is of non-integral type");
            }
            CHECK_OK(indvar_use->ReplaceOperandWith(i, constant_indvar));
          }
        }
      }
    }
...
StatusOr<bool> WhileLoopUnroller::Run(
  ...
  // need to perform sinking to fix-point
  HloPassFix<WhileLoopConstantSinking> constant_sinking(
      /*sink_broadcast_of_constants=*/true);
  TF_ASSIGN_OR_RETURN(bool sunk,
                      constant_sinking.Run(module, execution_threads));
  if (sunk) {
    VLOG(3) << "Applied constant sinking to module " << module->name();
  }

  // Gather all the while ops in our module. We only consider while loops that
  // take a tuple as argument. We do this ahead of time so we don't have to
  // worry about mutating the lists of computations or instructions while we
  // iterate.
  ...
  for (auto* comp : module->computations(execution_threads)) {
    for (auto* instr : comp->instructions()) {
      if (instr->opcode() == HloOpcode::kWhile) {
        ...
        if (ContainsInstrWithOpcode(
                instr->while_body(),
                {HloOpcode::kAllReduce, HloOpcode::kAllGather})) {
          VLOG(2) << "Not attempting to unroll " << instr->name()
                  << " for now because it contains a "
                     "all-reduce or all-gather: "
                  << instr->ToShortString();
          continue;
        }
        ...
        if (instr->while_condition()->HasSideEffect()) {
          VLOG(2)
              << "Not attempting to remove while loop whose condition contains "
                 "side-effecting instructions: "
              << instr->ToShortString();
          return false;
        }
        // TODO(b/291628533): Extract this to the unroller config
        if (instr->while_body()->instruction_count() > 800) {
          continue;
        }
  ...
  VLOG(3) << "Number of while instructions in the module: " << while_ops.size();
  ...
  for (HloInstruction* while_op : while_ops) {
    ...
    optional<int64_t> trip_count =
        PatternMatchLoopTripCount(while_op, *indvar_tuple_idx, indvar_iter_val);
    ...
    // loops that increase the instruction count by more than 10k.
    ...
    for (int64_t i = init_value.value();
      ...
      auto unrolled_body = module->AddEmbeddedComputation(
          LoopUnrollHelper(while_op, *indvar_tuple_idx, i).value());
