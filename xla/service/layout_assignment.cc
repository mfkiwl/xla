...
std::unique_ptr<Layout> LayoutAssignment::ChooseOperandLayoutFromOutputLayout(
  ...
  if (instruction->opcode() == HloOpcode::kReduce &&
      !instruction->shape().IsTuple()) {
    // Pick the operand layout that makes the reduce a row reduction. This is
    // only benefitial if the total size of reduction dims is large enough.
    int64_t reduction_size = 1;
    for (int64_t reduction_dim : instruction->dimensions()) {
      reduction_size *=
          instruction->operand(0)->shape().dimensions(reduction_dim);
    }
