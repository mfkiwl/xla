...
Status AlgebraicSimplifierVisitor::HandleDot(HloInstruction* dot) {
  ...
  if (options_.use_associative_reordering()) {
    ...
    if (lhs_to_rhs || rhs_to_lhs) {
      ...
      proto2::RepeatedField<int64_t> from_contracting_dims, to_contracting_dims;
      if (lhs_to_rhs) {
        from_contracting_dims = dnums.lhs_contracting_dimensions();
        to_contracting_dims = dnums.rhs_contracting_dimensions();
      } else {
        from_contracting_dims = dnums.rhs_contracting_dimensions();
        to_contracting_dims = dnums.lhs_contracting_dimensions();
      }
      for (int64_t i = 0; i < from_contracting_dims.size(); i++) {
        auto from_index = from_contracting_dims[i];
        auto to_index = to_contracting_dims[i];
      ...
      if (opcode == HloOpcode::kReverse) {
        ...
        if (!unreordered_dims.empty()) {
          ...
          TF_ASSIGN_OR_RETURN(unreordered,
                              MakeReverseHlo(reorder_from->mutable_operand(0),
                                             unreordered_dims));
        }
        ...
        for (int64_t to_dim = 0; to_dim < reorder_to->shape().rank();
             to_dim++) {
          ...
          if (contracting_dim_map_backward[to_dim] != -1) {
            ...
            int64_t from_dim = contracting_dim_map_backward[to_dim];
          ...
          if (dim.edge_padding_low() != 0 || dim.edge_padding_high() != 0) {
            ...
            TF_ASSIGN_OR_RETURN(unreordered,
                                MakePadHlo(reorder_from->mutable_operand(0),
                                           reorder_from->mutable_operand(1),
                                           new_padding_config));
            break;
          }
        }
        ...
        for (int64_t i = 0; i < from_contracting_dims.size(); i++) {
          int64_t from_index = from_contracting_dims[i];
          int64_t to_index = to_contracting_dims[i];
        ...
        if (make_hlo) {
          ...
          HloInstruction* zero =
              dot->AddInstruction(HloInstruction::CreateConstant(
                  LiteralUtil::Zero(reorder_from->shape().element_type())));

          // Create Hlo for unreordered broadcast and reordered reduce
          if (reorder_from->mutable_operand(0)->shape() !=
              new_broadcast_shape) {
            threshold_multiplier = 2.0;
            unreordered = MakeBroadcastHlo(reorder_from->mutable_operand(0),
                                           reorder_from->dimensions(),
                                           new_broadcast_shape);
          }
          TF_ASSIGN_OR_RETURN(
              reordered,
      ...
      if (make_hlo) {
        ...
        if (opcode == HloOpcode::kReverse) {
          const int64_t old_elements =
              ShapeUtil::ElementsIn(reorder_from->shape());
          const int64_t new_elements =
              ShapeUtil::ElementsIn(reorder_to->shape());
