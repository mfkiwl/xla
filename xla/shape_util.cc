...
/* static */ Shape ShapeUtil::MakeShapeWithDenseLayout(
    ...
    int64_t multiple_padded_to_in_elements, int64_t element_size_in_bits,
...
/* static */ Shape ShapeUtil::MakeShapeWithSparseLayout(
  ...
  auto ret = MakeShapeWithLayoutInternal(
      ...
      dim_ordered, /*tiles=*/{}, multiple_padded_to_in_elements,
...
ShapeUtil::MakeShapeWithDescendingLayoutAndSamePhysicalLayout(
  ...
  if (shape.has_layout()) {
    ...
    new_shape.mutable_layout()->set_multiple_padded_to_in_elements(
        shape.layout().multiple_padded_to_in_elements());
  }
  for (int i = 0; i < shape.dimensions_size(); ++i) {
...
Shape ShapeUtil::DeviceShapeToHostShape(Shape s) {
  ForEachMutableSubshape(&s, [](Shape* subshape, const ShapeIndex& index) {
    if (subshape->IsArray() && subshape->has_layout()) {
      ...
      subshape->mutable_layout()->set_multiple_padded_to_in_elements(1);
    }
  });
  return s;
}
...
/*static*/ int64_t ShapeUtil::ArraySize(const Shape& shape) {
  ...
  if (shape.layout().multiple_padded_to_in_elements() != 1) {
    num_of_elements = RoundUpTo(
        num_of_elements, shape.layout().multiple_padded_to_in_elements());
  }
